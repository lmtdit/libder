...
var Tools, _, _modName, _paths, _spaceName, build, color, fs, gulp, gutil, jsCtl, less, mincss, path, plumber, server, setting, watch,
...
jsCtl = (function() {
  ...
  jsCtl.prototype.madeModId = function(filepath) {
    return filepath.replace(/\\/g, '/').split(_modName + "/_src/")[1].replace(/.js$/, '');
  };
  ...
  jsCtl.prototype.init = function(file, done) {
    ...
    return this._stream(_file, function(obj, source) {
      ...
      return _this._buildJs(_distname, _source);
    }, function() {
...
build = {
  init: function() {
    var _dir, _jsData, _mainFile, _newPkg, e, i, init_dir, len, pkg;
    init_dir = [setting.jsPath, setting.imgPath, setting.lessPath, setting.tplPath];
  ...
  tpl2js: function(cb) {
    var _count, _htmlMinify, _tplPath, tplData, tpl_soure;
    ...
    return watch(_watchFiles, function(file) {
      ...
      try {
        ...
        if (_event !== 'undefined') {
          ...
          switch (_type) {
            case 'js':
              return new jsCtl().init();
            case 'less':
              return _this.less2js();
            case 'tpl':
              return _this.tpl2js();
          }
        }
